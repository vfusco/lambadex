...
for user in $(jq -r 'keys[]' "$OUT_FILE"); do
    ...
    for token_address in "$@"; do
        ...
        cast send $token_address "approve(address, uint256)(bool)" $ERC20_PORTAL_ADDRESS $DEPOSIT_AMOUNT --private-key="$key" #> /dev/null
        cast send $ERC20_PORTAL_ADDRESS "depositERC20Tokens(address, address, uint256, bytes)" \
            $token_address $DAPP_ADDRESS $DEPOSIT_AMOUNT "0x$payload_hex" --private-key="$key" --gas-limit=$GAS_LIMIT #> /dev/null
    done
    ((user_count++))
